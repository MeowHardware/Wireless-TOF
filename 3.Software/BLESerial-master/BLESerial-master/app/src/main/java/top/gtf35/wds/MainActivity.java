...
public class MainActivity extends AppCompatActivity implements BLESPPUtils.OnBluetoothAction, View.OnClickListener {
    ...
    public void onReceiveBytes(final byte[] bytes) {
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                if(Float.parseFloat(data) < 100){
                    postShowToast("距离过近，距离小于:" + new String(bytes) + "mm");
                    msgData.setTextColor(0xFFd71345);
                }else {
                    msgData.setTextColor(R.color.colorPrimaryDark);
                }
...
//        }
        ...
        if(v.getId() == R.id.btn_stop){
            if(stopButton.getText() == "继续"){
                mBLESPPUtils.send("c".getBytes());
                stopButton.setText("停止");
            }else{
                mBLESPPUtils.send("s".getBytes());
